...
def update_objets_trouves_a_bdd():
    ...
    last_date = datetime.strptime(last_date_string, '%Y-%m-%d %H:%M:%S') if last_date_string else datetime(2019, 1, 1 , 0, 0, 0)
    connexion.close()
    ...
    for gare in gares_parisiennes:
        ...
        response = requests.get(f"https://ressources.data.sncf.com/api/records/1.0/search/?dataset=objets-trouves-restitution&q=&rows=-1&sort=-date&facet=date&facet.range=date&facet=gc_obo_date_heure_restitution_c&facet=gc_obo_gare_origine_r_name&facet=gc_obo_nature_c&facet=gc_obo_type_c&facet=gc_obo_nom_recordtype_sc_c&refine.gc_obo_gare_origine_r_name={gare}")

        records = response.json()["records"]
        for record in records:
            # Ajouter le record à la liste all_records
            fields = record["fields"]
            date_string = fields["date"]
            date_string = re.sub(r'\+\d\d:\d\d$', '', date_string)  # Supprime le décalage horaire
